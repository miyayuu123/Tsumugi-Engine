...
class App:
    def __init__(self, url, desired_chars_per_cluster=5000, url_structure=None, query=''):
        self.urlmodule = URLModule(query)  # URLModule の初�� query を渡す
        ...
        self.query = query  # query をインスタンス変数として保持
...
def background_task(url, desired_chars_per_cluster, model_id, url_structure, query):
    ...
    app_instance = App(url, desired_chars_per_cluster, url_structure, query)  # App インスタンスを生成する際に query も渡す
...
def train_model():
    ...
    query = data.get('query', '')  # query パラメータを受け取る、デフォルトは空文字列
